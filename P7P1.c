	...
	if(pid==0){
		...
		while(i<3){
			...
			if((j=semop(semid,sops,nsops))==-1){
				...
				printf("\n\nProceso hijo toma el control de semaforo: %d/3 veces\n",i+1);
				sleep(5);
				nsops=1;
				sops[0].sem_num=0;
				sops[0].sem_op=-1;
				...
				else printf("Proceso hijo regresa el control del semaforo %d/3 veces\n",i+1);
				sleep(5);
			}
			++i;
		}
		...
		while(i<3){
			...
			sops[1].sem_op=1;
			sops[1].sem_flg=SEM_UNDO|IPC_NOWAIT;
			printf("semop: Padre llamado a semop(%d, &sops, %d) con:",semid,nsops);
			for(j=0;j<nsops;j++){
				printf("\n\tsops[%d].sem_num=%d, ",j,sops[j].sem_num);
				printf("sem_op=%d, ", sops[j].sem_op);
				printf("sem_flg=%#o\n", sops[j].sem_flg);
			...
			if((j=semop(semid,sops,nsops))==-1){
				...
				printf("\tsemop: regreso de semop()%d\n",j);
				printf("\n\nProceso Padre toma el control de semaforo: %d/3 veces\n",i+1);
				sleep(5);
				nsops=1;
				sops[0].sem_num=0;
				sops[0].sem_op=-1;
				...
				else printf("Proceso Padre regresa el control del semaforo %d/3 veces\n",i+1);
				sleep(5);
			}
			++i;
		}	
